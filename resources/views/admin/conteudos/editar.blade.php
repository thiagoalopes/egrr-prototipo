...
<form class="row g-3" action="{{ route('atualizar.conteudos') }}" method="POST">
    @csrf
    <input type="hidden" name="idCurso" value="{{ request()->get('idCurso') }}">
    <input type="hidden" name="idConteudo" value="{{ request()->get('idConteudo') }}">
    ...
    <div class="col-md-3">
      ...
      <input type="number"
      @if (old('sequencial_ordenacao') != null)
        value="{{ old('sequencial_ordenacao') }}"
      @else
        value="{{ $conteudo->sequencial_ordenacao }}"
      @endif
      placeholder="1-99" min="1" max="99" step="1" class="form-control @error("sequencial_ordenacao") is-invalid @enderror"  id="sequencial_ordenacao" name="sequencial_ordenacao">
    ...
    <div class="col-12">
      ...
      <textarea style="resize: none" required placeholder="Decreva o curso em até 500 caracteres" type="text" name="conteudo" maxlength="500" id="conteudo" rows="5" class="form-control @error("conteudo") is-invalid @enderror">@if (old('conteudo')!=null) {{ old('conteudo') }} @else {{ $conteudo->conteudo }} @endif</textarea>
      @error('conteudo')
        <span style="display: block;" class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
